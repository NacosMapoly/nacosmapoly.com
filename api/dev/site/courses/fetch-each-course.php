...
if ($apiKey!=$expected_api_key){
	...
	if (!empty($publish_id)){
		...
		$checkPageSession=$callclass->_check_page_session($conn, 'course_category', $publish_id, $page_session);
	...
	$select="SELECT 
		a.publish_id, a.reg_title, a.reg_pix, a.course_study_title, a.page_view, a.updated_time, b.seo_description, b.page_url, b.page_content, b.modified_by, c.fullname
		FROM publish_tab a, pages_tab b, staff_tab c
		WHERE a.page_category_id='course_category' 
		...
		if($all_record_count==0){
			...
			while ($fetch_query = mysqli_fetch_assoc($query)) {
				$fetch_query['documentStoragePath'] = "$documentStoragePath/course-pix";
